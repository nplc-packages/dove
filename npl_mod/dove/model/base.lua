...
function _M:ctor()
    if (self.resource_name == nil) then
        error("invalid model name: nil")
    end
    ...
    for attr, value in pairs(attributes) do
        ...
        if (attribute ~= nil) then
            if (type(value) ~= attribute.type) then -- TODO: auto convert
                error(
                    format("Invalid type of %s, expect is %s, but get %s", attribute.name, attribute.type, type(value))
                )
            end
            self[attr] = value
        end
...
function _M:save()
    if (self:is_create()) then
        self:execute_filters(self.before_create)
    end
    ...
    if (self:is_create()) then
        self:execute_filters(self.after_create)
    end
