...
local function restore_case(word, result)
    if (word == result or result == nil) then
        return result
    end
    ...
    if (word_len < result_len) then
        restored_word = restored_word .. result:sub(word_len + 1, result_len)
    end
...
function _M.is_plural(word)
    if (irregular_plurals[word] ~= nil) then
        return true
    end
    if (_M.check_uncountable_word_reg_rule(word)) then
        return true
    end
    if (_M.plural(word) == word) then
        return true
    end
    return false
end
...
function _M.is_singular(word)
    if (irregular_signles[word] ~= nil) then
        return true
    end
    if (_M.check_uncountable_word_reg_rule(word)) then
        return true
    end
    if (_M.singular(word) == word) then
        return true
    end
    return false
end
...
local function plural_word(word)
    ...
    if (irregular_signles[lower_word] ~= nil) then
        return restore_case(word, irregular_signles[lower_word])
    end
    if (_M.check_uncountable_word_reg_rule(lower_word) or irregular_plurals[lower_word] ~= nil) then
        return word
    end
...
local function singular_word(word)
    local result
    local lower_word = word:lower()
    if (irregular_plurals[lower_word] ~= nil) then
        return restore_case(word, irregular_plurals[lower_word])
    end
    if (_M.check_uncountable_word_reg_rule(lower_word) or irregular_signles[lower_word] ~= nil) then
        return word
    end
